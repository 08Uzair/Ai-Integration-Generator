  ...
  return (
    <div className="overflow-hidden rounded-xl border border-zinc-900">
      <table className="w-full text-left text-sm">
        <thead className="bg-zinc-950 text-xs uppercase tracking-wide text-slate-500">
          <tr>
        ...
        <tbody className="divide-y divide-zinc-900">
          ...
          {endpoints.map((ep, idx) => {
            ...
            return (
              <tr key={`${ep.method}-${ep.path}-${idx}`} className="bg-zinc-950/40 transition hover:bg-zinc-900/50">
                {isEditing ? (
                  <>
                    <td className="px-3 py-2">
                      <select
                        ...
                        className="rounded-lg border border-zinc-800 bg-zinc-900 px-2 py-1.5 font-mono text-xs text-slate-200"
                        aria-label="Method"
                      >
                    ...
                    <td className="px-3 py-2">
                      <input
                        ...
                        className="w-full min-w-32 rounded-lg border border-zinc-800 bg-zinc-900 px-2 py-1.5 font-mono text-xs text-slate-200"
                    ...
                    <td className="px-3 py-2">
                      <input
                        ...
                        className="w-full min-w-32 rounded-lg border border-zinc-800 bg-zinc-900 px-2 py-1.5 text-xs text-slate-200"
                        aria-label="Summary"
                      />
                    </td>
                    ...
                    <td className="px-3 py-2 text-right">
                      ...
                      <button type="button" onClick={() => setEditing(null)} aria-label="Cancel editing" className="rounded p-1 text-slate-500 transition hover:bg-zinc-800 hover:text-white">
                        <X size={14} />
                      </button>
                    </td>
                  </>
                  ...
                  <>
                    ...
                    {editable && (
                      <td className="px-3 py-2 text-right">
                        <button type="button" onClick={() => setEditing({ idx, method: ep.method, path: ep.path, summary: ep.summary || '' })} aria-label={`Edit ${ep.method} ${ep.path}`} className="rounded p-1 text-slate-500 transition hover:bg-emerald-500/10 hover:text-emerald-400">
                          <Pencil size={14} />
                        </button>
      ...
      {editable && (
        <div className="border-t border-zinc-900 bg-zinc-950/60 px-3 py-2.5">
          {adding ? (
            <div className="flex flex-wrap items-center gap-2">
              <select value={draft.method} onChange={(e) => setDraft({ ...draft, method: e.target.value })} className="rounded-lg border border-zinc-800 bg-zinc-900 px-2 py-1.5 font-mono text-xs text-slate-200" aria-label="Method">
                {METHODS.map((m) => <option key={m}>{m}</option>)}
              </select>
              <input
                value={draft.path}
                ...
                className="min-w-40 flex-1 rounded-lg border border-zinc-800 bg-zinc-900 px-2 py-1.5 font-mono text-xs text-slate-200 placeholder:text-slate-500"
                aria-label="Endpoint path"
              />
              <input
                value={draft.summary}
                ...
                className="min-w-40 flex-1 rounded-lg border border-zinc-800 bg-zinc-900 px-2 py-1.5 text-xs text-slate-200 placeholder:text-slate-500"
                aria-label="Summary"
              />
              <button type="button" onClick={addEndpoint} className="inline-flex items-center gap-1 rounded-lg bg-gradient-to-r from-emerald-500 to-green-500 px-3 py-1.5 text-xs font-medium text-white shadow-lg shadow-emerald-500/25 hover:brightness-110">
                <Plus size={13} /> Add
              </button>
              <button type="button" onClick={() => setAdding(false)} className="rounded-lg px-2 py-1.5 text-xs text-slate-400 hover:bg-zinc-900 hover:text-white">
                Cancel
              </button>
            </div>
          ) : (
            <div className="flex flex-wrap items-center gap-3">
              <button type="button" onClick={() => setAdding(true)} className="inline-flex items-center gap-1.5 rounded-lg px-2 py-1 text-xs text-slate-400 transition hover:bg-zinc-900 hover:text-white">
...
export function methodColor(method) {
  switch (method) {
    ...
    case 'POST': return 'bg-green-500/15 text-green-300 ring-green-500/30';
    case 'PUT':
    ...
    default: return 'bg-zinc-900 text-slate-400 ring-zinc-800';
  }
}

export function EndpointTableLoading() {
  return (
    <div className="flex items-center justify-center gap-2 rounded-xl border border-zinc-900 py-8 text-sm text-slate-400">
      <Loader2 size={15} className="animate-spin text-emerald-400" /> Analyzing endpoints...
    </div>
  );
}
...
export function EndpointTableEmpty() {
  return (
    <div className="flex items-center justify-center gap-2 rounded-xl border border-dashed border-zinc-800 py-8 text-sm text-slate-500">
