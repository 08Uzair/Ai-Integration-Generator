...
/**
 * Names of the path parameters inside a path, supporting every placeholder
 * style seen in the wild: `/users/:id`, `/users/{id}` and `/users/${id}`.
 * These MUST become part of the tool's inputSchema, otherwise the runtime
 * validator (Zod) strips them and the URL keeps a literal placeholder.
 */
function extractPathParams(path = '') {
  const names = [];
  const re = /:([A-Za-z_][A-Za-z0-9_]*)|{([A-Za-z_][A-Za-z0-9_]*)}/g;
  for (const match of String(path).matchAll(re)) {
...
function buildInputSchema(method, path, params, body, payload) {
  ...
  // Path placeholders are always part of the contract - even when the source
  // configuration did not declare them (manual endpoints, pasted client code).
...
function buildRequest(method, path, params, payload) {
  ...
  const hasBody =
    ['POST', 'PUT', 'PATCH'].includes(method) && !(payload && payload.hasBody === false);
  return { method, path, pathParams: extractPathParams(path), queryParams, body: hasBody };
}
